...
const SCAN_TIMEOUT_MS = 10 * 60 * 1000; // 10 minutes max scan time
...
// --- Tab close detection: abort scan if x.com tab is closed ---

chrome.tabs.onRemoved.addListener((tabId) => {
  if (tabId === activeScanTabId) {
    abortScan('Tab was closed during scan.');
  }
});
...
// --- Abort scan (shared cleanup) ---

async function abortScan(reason) {
  if (!activeScanTabId && !(await getScanState()).isScanning) return;
  ...
  // Best-effort: tell page scanner to stop
  if (activeScanTabId) {
    chrome.tabs.sendMessage(activeScanTabId, {
  ...
  scanTimeoutId = null;
  await updateScanState({ isScanning: false, progress: 0, activeScanTabId: null });
  activeScanTabId = null;
  scanFollowers = [];

  broadcastToPopup({ type: MSG.SCAN_ERROR, error: reason });
}
...
async function handleScanFinished(message) {
  ...
  try {
    ...
    broadcastToPopup({
      ...
      isFirstScan,
      expectedCount: message.expectedCount || null,
...
async function handleMessage(message, sender) {
  switch (message.type) {
    ...
    case MSG.CANCEL_SCAN: {
      await abortScan('cancelled');
      return { cancelled: true };
    }

    // Page world scan events — only accept from the active scan tab
    case MSG.SCAN_BATCH:
    case MSG.SCAN_PAGE_DONE:
    case MSG.SCAN_FINISHED:
    case MSG.SCAN_PAGE_ERROR: {
      // Restore in-memory tabId from storage if service worker restarted
      if (!activeScanTabId) {
        const st = await getScanState();
        if (st.activeScanTabId) activeScanTabId = st.activeScanTabId;
      }
      if (sender.tab?.id && sender.tab.id !== activeScanTabId) {
        return { ok: false, error: 'wrong_tab' };
      }
      if (message.type === MSG.SCAN_BATCH) return handleScanBatch(message);
      if (message.type === MSG.SCAN_PAGE_DONE) return handleScanPageDone(message);
      if (message.type === MSG.SCAN_FINISHED) return handleScanFinished(message);
      if (message.type === MSG.SCAN_PAGE_ERROR) return handleScanPageError(message);
      break;
    }
    ...
    case MSG.GET_STATUS: {
      ...
      // Detect stale scan: isScanning but tab gone or started too long ago
      if (scanState.isScanning) {
        const staleSince = scanState.startedAt
          ? Date.now() - scanState.startedAt > SCAN_TIMEOUT_MS
          : true;

        // Check if the scan tab is still alive (use stored ID as fallback)
        const tabId = activeScanTabId || scanState.activeScanTabId;
        let tabGone = !tabId;
        if (tabId && !tabGone) {
          try { await chrome.tabs.get(tabId); } catch { tabGone = true; }
        }

        if (staleSince || tabGone) {
          await abortScan('Scan session expired.');
          return {
            isScanning: false,
            progress: 0,
    ...
    // Content script asks if it should resume a scan after page navigation
    case MSG.CHECK_SCAN_PENDING: {
      const state = await getScanState();
      const storedTabId = activeScanTabId || state.activeScanTabId;
      const isActiveTab = sender.tab?.id === storedTabId;
      if (state.isScanning && isActiveTab) {
        // Restore in-memory state if service worker had restarted
        if (!activeScanTabId) activeScanTabId = storedTabId;
        // Reset timeout since scan is resuming
        clearTimeout(scanTimeoutId);
        scanTimeoutId = setTimeout(() => {
          abortScan('Scan timed out after 10 minutes.');
        }, SCAN_TIMEOUT_MS);
